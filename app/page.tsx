...
/* =====================
  型定義（route.ts は string 受けなので、UI側は増やしてOK）
===================== */
type JobKey =
  | "SALES_NEW"
  | "SALES_ROUTE"
...
/* =====================
  ラベル
===================== */
function labelJob(key: JobKey | "") {
  if (!key) return "";
  for (const g of JOB_GROUPS) {
    const hit = g.options.find((o) => o.value === key);
    if (hit) return hit.label;
  }
  return "その他";
}
function labelIndustry(key: IndustryKey | "") {
  if (!key) return "";
  return INDUSTRY_OPTIONS.find((o) => o.value === key)?.label ?? "その他";
}
function labelTenure(key: TenureKey | "") {
  if (!key) return "";
  return TENURE_OPTIONS.find((o) => o.value === key)?.label ?? "";
}
...
export default function Home() {
  ...
  const payload = useMemo(
    () => ({
      ...
      // 互換用（route.ts が job/industry 参照でもOK）
      job: labelJob(jobKey),
  ...
  return (
    <main className="min-h-screen bg-white text-zinc-900">
      <div className="mx-auto max-w-3xl px-4 py-10 space-y-6">
        ...
        {/* ✅ ここに必ず出る：UP-STREAM フッター */}
        <footer className="pt-6 pb-2 text-center text-xs text-zinc-500">
